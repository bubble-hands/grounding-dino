...
def generate_fingerprint(model, prompt_texts, model_type="raw"):
    """生成模型溯源指纹"""
    params_hash = hashlib.md5()
    for name, param in model.named_parameters():
        params_hash.update(param.data.cpu().numpy().tobytes()[:1000])
    
    prompt_hash = hashlib.md5()
    for text in prompt_texts[:50]:
        prompt_hash.update(text.encode('utf-8'))
    
    fingerprint = {
        'timestamp': datetime.datetime.now().isoformat(),
        'model_type': model_type,
        'params_hash': params_hash.hexdigest()[:16],
        'prompt_hash': prompt_hash.hexdigest()[:16],
        'prompt_count': len(prompt_texts),
        'fingerprint_id': f"{params_hash.hexdigest()[:8]}_{prompt_hash.hexdigest()[:8]}"
    }
    return fingerprint
...
def draw_single_pred_box(img, pred_box, pred_score, orig_size, color=(0, 255, 0)):
    h, w = img.shape[:2]
    result = img.copy()
...
def draw_comparison(rgb_img, pred_box, pred_score, annotations, orig_size, text_label, iou_score, model_type):
    h, w = rgb_img.shape[:2]
    gap = 10
    panel_width = w
    panel_height = h
    ...
    cv2.putText(canvas, f'Prediction ({model_type})', (panel_width + gap + 10, 30),
    ...
    pred_img = draw_single_pred_box(rgb_img, pred_box, pred_score, orig_size, color=pred_color)
    ...
    score_text = f'Score: {float(pred_score):.3f}  |  IoU: {iou_score:.3f}  |  Model: {model_type}'
    cv2.putText(canvas, score_text, (canvas_width - 450, info_y),
               cv2.FONT_HERSHEY_SIMPLEX, 0.65, pred_color, 2)
    
    return canvas


def main():
    parser = argparse.ArgumentParser(description='Baseline Inference - Raw Model (No Fine-tuning)')
    parser.add_argument('--config', type=str, 
                        default='groundingdino/config/GroundingDINO_SwinT_MultiModal.py')
    parser.add_argument('--data', type=str, default='./data', help='Data directory')
    parser.add_argument('--output', type=str, default='./baseline_results', help='Output directory')
    ...
    parser.add_argument('--model_type', type=str, default='raw',
                        ...
                        help='Model type: raw (no weights) or fine_tuned (with weights)')
    parser.add_argument('--checkpoint', type=str, default=None,
                        help='Path to checkpoint (only for fine_tuned mode)')
    ...
    print(f"{'='*60}")
    print(f"Baseline Inference - {'Raw Model (No Fine-tuning)' if args.model_type == 'raw' else 'Fine-tuned Model'}")
    print(f"{'='*60}")
    ...
    if args.model_type == 'fine_tuned' and args.checkpoint:
        ...
        if os.path.exists(checkpoint_path):
            ...
            print("Using raw model instead...")
            args.model_type = 'raw'
    else:
        print("Using RAW model (random initialization, no fine-tuning)")
    ...
    all_prompts = [item.get('text', '') for item in val_data]
    fingerprint = generate_fingerprint(model, all_prompts, args.model_type)
    print(f"\nModel Fingerprint:")
    print(f"  ID: {fingerprint['fingerprint_id']}")
    print(f"  Type: {fingerprint['model_type']}")
    print(f"  Params hash: {fingerprint['params_hash']}")
    print(f"  Prompt hash: {fingerprint['prompt_hash']}")
    print(f"  Prompt count: {fingerprint['prompt_count']}")
    ...
    model_output_dir = os.path.join(args.output, args.model_type)
    ...
    for idx in range(num_samples):
        ...
        if os.path.exists(rgb_path) and orig_size is not None:
            ...
            comparison_img = draw_comparison(rgb_img, pred_box, pred_score, annotations, 
                                           orig_size, text, iou_score, args.model_type)
            save_path = os.path.join(model_output_dir, f'sample_{idx:03d}.jpg')
            ...
            if gt_box_orig:
                print(f"  GT box (px): [{gt_box_orig[0]:.0f}, {gt_box_orig[1]:.0f}, {gt_box_orig[2]:.0f}, {gt_box_orig[3]:.0f}]")
    ...
    print(f"Model Fingerprint ID: {fingerprint['fingerprint_id']}")
